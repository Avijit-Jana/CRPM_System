...
def webapp():
    ...
    with tab1:
        ...
        with col2:
            ...
            product_name = st.text_input('Product Name', placeholder='e.g. iPhone 13', key="product_name1")
        ...
        customer_values = [customer_name, gender, age, country, state, city, str(order_date), phone, email, order_status, product_name, quantity, address]

        if st.button('Submit', key='customer'):
            if all(customer_values):
                d = Database()
                d.insert_customer(customer_values)
    ...
    with tab2:
        ...
        product_values = [product_name, stock, color, category, cost_price, discount, brand, unit_price, product_status, product_description]

        if st.button('Submit', key='product'):
            if all(product_values):
                st.info('Product Information Submitted')
                d1 = Database()
                d1.insert_product(product_values)
            else:
                st.error('Please fill in all fields.')
